    ...
    module Command
      class ManGames
        ...
        def call
          ...
          message << 'Результаты поиска сохраняются в течении двух минут, и если игрок за это ' \
          ...
          message << 'Стоит помнить, что HolyRider запоминает игрока, отправившего команду ' \
                      '<code>/games</code>, поэтому получить информацию об игре из списка может только игрок, отправивший команду.'
          message << 'То есть, если другой игрок захочет найти несколько игр, то ему надо ввести команду <code>/games</code> самому'
